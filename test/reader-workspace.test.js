...
test('native article page exposes explicit Shervin body enrichment with persistent offline queue semantics',()=>{
 ...
 assert.match(ui,/const noun=isTranscript\?'字幕':'正文'/);assert.match(ui,/isTranscript\?'字幕需要 B站登录 · 点击重试':'Shervin 需要重新登录'/);assert.match(ui,/任务会保留，Shervin 再次运行 collector 后继续/);
 assert.match(ui,/isTranscript\?'获取字幕':'补正文'/);assert.match(ui,/Shervin 已补充'\+noun/);
