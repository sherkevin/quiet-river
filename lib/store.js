...
// 随仓库发布的缓存快照：clone 后第一次启动、本地还没有 cache.json 时用它垫底，
// 页面立刻有内容可看。能抓的源会在第一次刷新后被真实结果覆盖；登录态源在
// 没有凭证的机器上永远停在快照时刻——README 里写清了这一点。
const SEED_CACHE_PATH = path.join(DATA_DIR, 'cache.seed.json');
...
function loadCache() {
  return readJson(CACHE_PATH, null).then((cache) => {
    if (cache) return cache;
    return readJson(SEED_CACHE_PATH, EMPTY_CACHE);
  });
}
...
/**
 * 把一个 resolve() 的结果落到订阅对象上（PATCH 改 feed 地址用）。
 *
 * 关键点：adapter 必须「命中就换、不命中就删」。两条刷新路径都是
 * 「有 sub.adapter 就走 adapter，没有才抓 feeds」（见 lib/refresh.js），
 * 所以只写 `if (result.adapter) sub.adapter = ...` 而不清理的话，把一个
 * 适配器源（arXiv cs.IR）改成普通 feed 地址，刷新时仍旧走旧适配器，
 * 改地址成了空操作，页面上看不出任何变化。
 */
function applyResolvedFeed(sub, result, raw) {
  // 转发服务的地址只进 feeds，主页链接仍是作者主页——和 POST 一致，
  // 否则在编辑弹窗里补一次 feed 地址，「去主页」就跳去 api.xgo.ing 了。
  sub.url = result.homepage || raw;
  sub.feeds = result.feeds.filter((f) => f.ok).map((f) => f.url);
  if (result.adapter) sub.adapter = result.adapter;
  else delete sub.adapter;
