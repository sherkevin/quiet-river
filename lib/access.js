...
// 跨域客户端（例如把网页托管在别处、数据走中继的形态）带不了本站 cookie，
// 允许用请求头传口令。浏览器 fetch 对自定义头会先发 CORS 预检，所以下面
// 的 CORS 处理要放行它。
const TOKEN_HEADER = 'x-qr-token';
...
function guard(req, res, url, sendJson) {
  ...
  const fromHeader = String(req.headers[TOKEN_HEADER] || '').trim();
  const presented = fromQuery || fromCookie || fromHeader;
  const hadSomething = Boolean(fromQuery || fromCookie || fromHeader);
  if (!sameSecret(presented, token)) {
    ...
    sendLogin(res, hadSomething ? 403 : 200, hadSomething ? '口令不对，再试一次。' : '');
    return false;
  }
