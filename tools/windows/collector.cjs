...
async function main(){
  if(args.includes('--help')){console.log('collector.cjs [--watch] [--max-jobs 20] [--platform zhihu|xiaohongshu|bilibili|twitter|instagram|reddit] [--doctor] [--verify-twitter HANDLE] [--verify-twitter-opencli HANDLE]');return;}
  ...
  const platforms=option('--platform','zhihu,xiaohongshu,bilibili,twitter,instagram,reddit').split(',').filter(p=>['zhihu','xiaohongshu','bilibili','twitter','instagram','reddit'].includes(p));
