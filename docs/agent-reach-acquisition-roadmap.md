...
| WeChat | 65 | 49 existing third-party feeds; 16 verified MP identities remain metadata-only/unrefreshed because WeRSS is intentionally skipped |
...
Deployment gate: WeRSS was explicitly skipped by user decision and is no longer a release dependency. Production readiness is now determined only by the acquisition feature's own tests/canaries and normal release smoke.
...
- existing WeChat public/relay feeds; the 16 unresolved MP identities stay metadata-only until a non-WeRSS source is found
...
- Platform work is consolidated on `chatgpt/acquisition-release-v1`; WeRSS is intentionally excluded from the release line.
