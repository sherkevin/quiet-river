...
- decision — WeChat acquisition after skipping WeRSS has been audited. Current inventory remains 65 total: 49 working public/relay feeds (40 BestBlogs Wechat2RSS + 9 xlab) and 16 unresolved metadata-only sources. Exact checks against BestBlogs' 375-source public OPML and xlab's current public complete list found 0/16 coverage; BestBlogs fuzzy-name audit found no credible alias matches. Detailed comparison is in `docs/wechat-acquisition-options.md`. Policy: do not reintroduce WeRSS and do not deploy private Wechat2RSS yet only for these 16. Prefer existing public providers; private Wechat2RSS becomes the next candidate only if the unresolved sources are must-have and the user accepts a maintained WeRead-authorized account plus license cost.

- doing — WeChat public-provider recovery is now the next low-cost path. `docs/wechat-provider-requests.md` lists the 16 unresolved accounts, xlab/BestBlogs inclusion channels, required submission data and acceptance gates. Public pool audit remains 0/16. Do not deploy private Wechat2RSS until public inclusion has been tried and the remaining sources are confirmed must-have.

- decision gate — Private Wechat2RSS is inexpensive (CNY 15/month or 150/year) but is not free operationally: it requires at least one maintained WeRead-authorized WeChat account and official deployment guidance recommends >=512 MiB RAM. Current ECS has ~1.7 GiB total / ~462 MiB available, 2 GiB swap unused, Karakeep ~364 MiB and Miniflux ~43 MiB, so adding Wechat2RSS here would be memory-tight. Do not deploy it merely because 16 sources are missing. Reconsider only if several of those 16 are confirmed must-have and public-provider inclusion still fails; then prefer an ECS memory upgrade or a separate small instance. Detailed gate appended to `docs/wechat-acquisition-options.md`.
