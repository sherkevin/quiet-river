...
- done on feature branch, not deployed — Podcast transcription on `chatgpt/podcast-transcript-v1` keeps standard RSS as discovery and adds explicit per-entry Shervin-local faster-whisper enrichment. No Groq/OpenAI/cloud audio upload is used. Full Recsperts canary: 84.6 min audio -> 12.6 min local processing (RTF ~0.149), 409 segments / 75,605 chars, ~500 MiB memory, and 0 temp audio/WAV files left. Focused tests 152/152; full regression 358/358 before docs-only updates. Production DB was not mutated.
