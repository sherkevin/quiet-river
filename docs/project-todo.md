...
- done on dedicated feature branch, not deployed — Reddit Community source implemented on `chatgpt/reddit-source-v1` as `reddit.community.posts -> reddit-rss-shervin`. Discovery uses official Reddit Atom RSS through Shervin Browser Bridge with the RSS fetch forced to `credentials:'omit'`; no Reddit account/login/Cookie is required. Stable item ID is native `t3_<post_id>`, comments remain future detail enrichment, and cross-community identity is rejected on both worker/ECS boundaries. Final `r/LocalLLaMA` wrapper canary: 5/5 valid rows, unique t3 IDs, valid timestamps, stderr 0. Focused 129/129 and full 343/343 passed. Reddit User source remains deferred pending a separate zero-account canary.
