...
>
> 现在实际在用的是 **wechat2rss 免费公开目录**，而且**有两个实例**，必须两个都查：
> `https://wechat2rss.xlab.app/list/all/`（395 个号，每源 20 条）和
> BestBlogs 仓库的 `opml/bestblogs_wechat2rss_opml_all.opml`（375 个号，域名
> `wechat2rss.bestblogs.dev`，每源 10 条）。**两者只重叠 23 个名字**，是互补关系。
> 2026-09-14 先用第一个把公众号从 18 个做到 26 个（其中 9 个有内容）；
> 同日发现第二个实例后，又从它入库 45 个，公众号到 71 个。
> 关键差别是**大厂技术号基本只在第二个里**：快手技术、大淘宝技术、小红书技术REDtech、
> 百度Geek说、蚂蚁技术、京东技术、腾讯技术工程、滴滴技术、vivo互联网技术、携程技术。
> 下面原文保留作历史记录，**其中「建议先试 wewe-rss」的推荐作废**，
> 「目录只有 xlab.app 一个」这个隐含前提也作废。
...
`lib/resolve.js` 里也加了按点边界匹配两个实例域名的识别（`relayOf`），
否则这些源会被误判成 `blog`。

### 第二个实例：BestBlogs 的 wechat2rss（2026-09-14 发现并入库）

[ginobefun/BestBlogs](https://github.com/ginobefun/BestBlogs)（4023★）是 bestblogs.dev 的
公开镜像仓，`opml/` 下放着一整套现成 OPML。其中
`bestblogs_wechat2rss_opml_all.opml` 是**另一个 wechat2rss 实例**（`wechat2rss.bestblogs.dev`）
的 375 个号，和 xlab.app 那个 395 个号的目录**只重叠 23 个名字**。

实测（2026-09-14，全部走项目自己的 `verifyFeed`）：抽验 12 个全部 HTTP 200、
每源 10 条、`pubDate` 在数天内、标题是真文章（快手技术首页那条就是 OneRec）。
据此入库 45 个，判据是本仓 `docs/决议/0004` 那条「收『在做』的，不收资讯转述」
（决议目录不随开源版发布），分四类：

| 类别 | 入库的号 |
|---|---|
| 大厂技术号（搜广推/大模型一手） | 快手技术、大淘宝技术、小红书技术REDtech、百度Geek说、蚂蚁技术AntTech、京东技术、腾讯技术工程、阿里云开发者、滴滴技术、vivo互联网技术、携程技术、Qunar技术沙龙、转转技术、字节跳动Seed、字节跳动开源、通义实验室、腾讯混元、月之暗面 Kimi、DeepSeek、智谱、MiniMax、阶跃星辰、Xiaomi MiMo、支付宝体验科技、有赞coder |
| AI/ML 媒体与论文解读 | AINLP、PaperWeekly、AI前线、机器之心SOTA模型、Datawhale、魔搭ModelScope社区 |
| 架构 / 数据 / 工程 | 高可用架构、架构师之路、dbaplus社群、稀土掘金技术社区、数据可视化 AntV、阿里研究院 |
| 数理 / 科学 / 金融量化 | 集智俱乐部、返朴、中科院物理所、神经现实、中金点睛、香帅的金融江湖、格兰投研、饭统戴老板 |

**两个实例的差别要记着**：bestblogs 那个每源只返回 10 条（xlab 是 20 条），
且走 Cloudflare 缓存（`max-age=600`）。所以 `lib/refresh.js` 给它单独设了
半小时重抓周期与 1 秒请求间隔。

**2026-09-18 身份补全：原先这批“17 个无通道公众号”已经不再是纯手动占位。**
通过“搜狗文章索引的来源公众号 → Shervin 浏览器打开微信原文 → 校验页面 `#js_name` →
读取 `window.biz` → Base64 反解 `MP_WXS_<数字>`”的证据链，16 个真实公众号都已绑定稳定
`mp_id`，直接写在各自的 `source.adapter` 中；这些 ID 不依赖另一个环境变量映射表。

原来的「丁丁丁写字的地方」经旧来源名跳转到真实微信原文后，当前公众号名为
「推广搜老油条」，因此保留原 source ID 与 tags，只更新显示名和 `mp_id`。
「搜广推学习笔记」则没有可核验的同名公众号：公开证据只稳定指向
`Wzy1007007/recommend-system-note` 的课程笔记仓库标题，且搜狗文章前 10 页没有同名来源公众号，
所以按“来源必须是实际博主/账号”的原则从订阅清单删除，而不是伪造公众号 ID。

注意：**身份已补全不等于 WeRSS 采集服务已上线。** 当前 Bridge 能为这些 `MP_WXS_*`
生成确定的 WeRSS channel；若服务器尚未配置 WeRSS runtime，channel 必须保持
`NOT_CONFIGURED`/disabled，不能把“知道公众号是谁”误报成“已经能自动更新”。
