...
function drawNativeArticle(container,detail){
  ...
  const prepare=shell.querySelector('[data-native-prepare]');if(!detail.canFetchFullText)prepare.remove();else{const prepareLabel=detail.prepareKind==='youtube-transcript'?'获取字幕':detail.prepareKind==='github-detail'?'获取 GitHub 详情':detail.prepareKind==='bilibili-detail'?'获取视频详情':'尝试补全文';prepare.textContent=prepareLabel;prepare.onclick=async()=>{prepare.disabled=true;prepare.textContent=detail.prepareKind==='youtube-transcript'?'正在获取字幕…':detail.prepareKind==='github-detail'?'正在获取 GitHub 详情…':detail.prepareKind==='bilibili-detail'?'正在获取视频详情…':'正在补全文…';try{const next=await api('/entries/'+detail.id+'/prepare',{});next.highlights=detail.highlights||[];next.highlightConfigured=detail.highlightConfigured;next.highlightCanCreate=next.contentState!=='META'&&detail.highlightConfigured;next.bodyEnrichment=detail.bodyEnrichment;drawNativeArticle(container,next);toast(next.prepareImproved?'已补充更完整内容':'没有取得比当前更完整的内容');}catch(e){error(e);prepare.disabled=false;prepare.textContent=prepareLabel;}};}
  setupNativeEnrichment(shell,container,detail);
