...
function setupNativeTranscript(shell,container,detail){
  const button=shell.querySelector('[data-native-transcript]'),initial=detail.youtubeTranscript;
  if(detail.platform!=='youtube'||!initial?.eligible){button.remove();return;}
  let polling=false,stopped=false;
  const paint=state=>{
    detail.youtubeTranscript=state;const online=state.collector?.online;
    if(state.state==='DONE'){button.disabled=true;button.textContent='已补充视频字幕';button.title='字幕已经写入站内正文';}
    else if(state.state==='RUNNING'){button.disabled=true;button.textContent='Shervin 正在读取字幕…';}
    else if(state.state==='QUEUED'){const retry=Number(state.next_attempt)>Date.now()&&!!state.error;button.disabled=!retry;button.textContent=retry?'字幕受限 · 点击立即重试':online?'字幕已排队…':'字幕已排队 · Shervin 离线';button.title=state.error||'任务会保留，Shervin 上线后继续';}
    else {button.disabled=false;button.textContent=online?'补充视频字幕':'Shervin 离线 · 先排队字幕';button.title='优先使用 yt-dlp；失败后按既定链路回退到 OpenCLI。不会下载视频。';}
  };
  const reload=async state=>{
    ...
    next.highlights=hs.highlights||[];next.highlightConfigured=hs.configured!==false;next.highlightCanCreate=!!hs.canCreate;next.bodyEnrichment=detail.bodyEnrichment;next.youtubeTranscript=state;drawNativeArticle(container,next);toast('视频字幕已加入站内正文');
  };
  const poll=async()=>{
    if(polling||stopped)return;polling=true;
    try{for(let i=0;i<60&&!stopped;i++){await wait(3000);const state=await api('/entries/'+detail.id+'/transcript');paint(state);if(state.state==='DONE'){stopped=true;await reload(state);break;}if(['UNAVAILABLE','FAILED'].includes(state.state)){stopped=true;break;}}}
    catch(e){error(e);}finally{polling=false;}
  };
  paint(initial);if(['QUEUED','RUNNING'].includes(initial.state))void poll();
  button.onclick=async()=>{button.disabled=true;try{const state=await api('/entries/'+detail.id+'/transcript',{});paint(state);void poll();toast(state.collector?.online?'字幕任务已提交给 Shervin':'字幕任务已排队；Shervin 上线后继续');}catch(e){paint(detail.youtubeTranscript);error(e);}};
}
...
function drawNativeArticle(container,detail){
  ...
  const prepare=shell.querySelector('[data-native-prepare]');if(!detail.canFetchFullText||(detail.contentState==='TEXT'&&detail.contentTextLength>=1500))prepare.remove();else prepare.onclick=async()=>{prepare.disabled=true;prepare.textContent='正在补全文…';try{const next=await api('/entries/'+detail.id+'/prepare',{});next.highlights=detail.highlights||[];next.highlightConfigured=detail.highlightConfigured;next.highlightCanCreate=next.contentState!=='META'&&detail.highlightConfigured;next.bodyEnrichment=detail.bodyEnrichment;next.youtubeTranscript=detail.youtubeTranscript;drawNativeArticle(container,next);toast(next.prepareImproved?'已补充更完整正文':'没有取得比当前更完整的正文');}catch(e){error(e);prepare.disabled=false;prepare.textContent='尝试补全文';}};
...
async function renderArticleDetail(id){
  ...
  if(detail.platform==='youtube'){try{detail.youtubeTranscript=await api('/entries/'+id+'/transcript');}catch{detail.youtubeTranscript={eligible:false,state:'UNAVAILABLE'};}}else detail.youtubeTranscript={eligible:false,state:'UNAVAILABLE'};
