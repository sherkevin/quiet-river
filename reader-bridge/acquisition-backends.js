...
registerProbe('opencli-reddit-user-shervin',async service=>{const status=service.desktop?.status?.();return status?.online?{status:'warn',reason:'Shervin collector is online; zero-account Reddit user-posts availability is confirmed per source check',state:'UNVERIFIED'}:{status:'off',reason:'Shervin collector is offline',state:'OFFLINE'};});
