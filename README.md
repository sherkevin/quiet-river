...
**clone 下来就是满的。** 仓库里带一份订阅清单（`data/subscriptions.json`，170 个源）和一份缓存快照（`data/cache.seed.json`，每源最近 20 条）：第一次启动不用等抓取，页面立刻有内容。能匿名抓的源（博客、GitHub、arXiv、CSDN 等）会在第一次自动刷新后换成实时内容；需要登录态的源（知乎、小红书）在你没有凭证的机器上停在快照时刻——它们是「读档」，不是「活的」。想清空重来：删掉 `data/` 下两个 json，或点页面上的删除逐个清。
...
| 3 | 平台需要登录态凭证，或贴主页链接识别不出来 | 知乎、小红书（本仓库不提供适配器，见下）；微信公众号（手动登记或走 wechat2rss 公开目录）；微博；X/Twitter（任意主页无解，但有第三方现成 feed 池，见下） |

第 2 档在「设置」里填 `rsshubBase`（比如 `http://127.0.0.1:1200`）后生效。**社区公共镜像不要指望**：2026-09-10 实测 8 个镜像 0 个可用（2 个 503、2 个被域名拦截、4 个连不通），唯一返回 RSSHub 错误页的那个还暴露了它没装 Playwright chromium——B站视频路由在匿名被风控时正是要靠 chromium 回退。**B站只能自托管**。

2026-09-14 补一个例外：`https://rsshub.bestblogs.dev`（BestBlogs 自建实例）实测活着，掘金、少数派、小宇宙、知乎日报、HN 路由都能直出 feed，所以 `rsshubBase` 可以先填它当兜底。但它的 **B站路由全部超时**（没装 chromium），且它是别人为自家产品跑的实例、不是公共服务，随时可能加白名单——当兜底可以，当依赖不行。
...
目前唯一在用的免费外部渠道是 [wechat2rss 公开目录](https://wechat2rss.xlab.app/list/all/)：收录 395 个公众号，每个直接给标准 RSS 地址，免登录、免部署、免付费，贴回来就能抓（`lib/resolve.js` 认这个域名，会正确归到「微信公众号」而不是「博客」）。代价是覆盖率低且以安全类为主，命中不了就还是手动登记。
...
## 第三方现成 feed 池：能用，但别当基础设施

有些平台我们抓不到，**别人已经替我们抓好了并且公开了 feed 地址**。这类通道和「自托管」不一样：不用部署、不用登录态，`curl` 就能拿。2026-09-14 实测三类都活着。

| 平台 | 端点 | 实测 | 边界 |
|---|---|---|---|
| X / Twitter | `api.xgo.ing/rss/user/<id>` | 匿名直出标准 RSS 2.0，每账号 50 条，带 `pubDate`。随机抽 8 个账号 8 通过 0 失败 | **只能用别人清单里现成的账号**：id 是不透明 hash（不是 handle 的 md5），`/rss/user/karpathy` 这种写法 404，无法自增 |
| 微信公众号 | `wechat2rss.bestblogs.dev/feed/<sha1>.xml` | 与 `wechat2rss.xlab.app` 同一套代码的不同实例，375 个号，取向是 AI / 大厂研发 / 商业科技 | **没有目录页**（`/list/all/` 404），只能按地址直连；地址要从上游 OPML 取 |
| 掘金 / 少数派 / 小宇宙 | `rsshub.bestblogs.dev/<路由>` | 见第 2 档说明 | B站路由全部超时 |

两个 wechat2rss 实例**互补**：feed hash 零重叠（同一个号在两边是不同 URL），公众号名字只重叠 23 个，**并集约 748 个**——比单看 xlab.app 的 395 个宽得多。

**为什么不内置成默认配置。** 这三个端点都是别人为自家产品跑的，没有 SLA，XGo 本身还是付费产品（Free / Plus $13 / Pro $69 每月），RSS 端点公开免费属于「别人请客」，随时可能收进付费墙或加 IP 白名单。把订阅建在上面，等于把可用性外包给陌生人。所以本项目只用它们的 **feed URL 这类事实性数据**（URL 不受版权保护），地址清单也不随本仓发布——上游 `ginobefun/BestBlogs` 仓库没有 LICENSE 文件、`license` 字段为空，法律上未授权复制，而且跟着我们公开几百个第三方源地址等于替对方承担分发责任。要用的人自己去上游取，取到之后：

```bash
# OPML 或 名字<TAB>地址<TAB>tag 文本，批量灌进来
node tools/import-feeds.js feeds.opml --tags 生成式推荐
```

还有一条要提醒：这些通道的**覆盖率对你的清单可能毫无帮助**。本项目作者的 17 个手动登记公众号（多是个人学习笔记号）在 375 个里精确命中 0、模糊命中 0；X 那个 160 账号的池子里也没有一个推荐系统或搜索广告方向的账号，全是模型侧的大号。「能抓到」不等于「该收」，收录标准仍然看专业性与是否在兴趣域内。
