...
**clone 下来就是满的。** 仓库里带一份订阅清单（`data/subscriptions.json`，276 个源）和一份缓存快照（`data/cache.seed.json`，每源最近 20 条）：第一次启动不用等抓取，页面立刻有内容。能匿名抓的源（博客、GitHub、arXiv、CSDN、公众号与 X 的转发地址等）会在第一次自动刷新后换成实时内容；需要登录态的源（知乎、小红书）在你没有凭证的机器上停在快照时刻——它们是「读档」，不是「活的」。想清空重来：删掉 `data/` 下两个 json，或点页面上的删除逐个清。
