...
由你选择关注对象，系统收集更新并保存内容；**推荐算法和日报选取在自己的服务器上执行**。
新入口是 `/desk/`，提供优先阅读、全部更新、日报、来源状态和阅读偏好；
正文、划线和批注由自托管 Karakeep 保存。没有全文时保留元信息与原文入口。

固定组合：Miniflux + PostgreSQL + Karakeep 最小安装 + Reader Bridge + Caddy + ntfy。
Bridge 需要 Node.js 22.13 或更新的兼容版本（当前验证为 22.23.2）。
这不是运行 `node server.js` 就能启动的同一个应用；部署入口在 `deploy/`。

- [实际实施状态、运行验证和仍未通过的验收项](docs/implementation-status-20260917.md)
- [Bridge 说明](docs/reader-platform-implementation.md)
- [完整方案](docs/private-reader-platform-impl-v1.md)

**自动发现更新、全文可用、批注可用是不同状态。** 尚未配置的平台凭证和适配器仍需接通，
不把历史快照或作者主页当作自动订阅已完成。既有逐作者第三方 Feed 不因其第三方性质被排除。

## 旧版时间轴与兼容说明

以下文档描述原来的零依赖时间轴。生产切换后它作为 `/legacy/` 只读保留；
“不做推荐、不存全文”是旧版边界，不再限制新平台的产品目标。
